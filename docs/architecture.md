The diff removes the `Onboarding.tsx`, `Posts.tsx`, and `app/posts/[slug]/page.tsx` files, and updates `app/[slug]/page.tsx` to return a 404 instead of a `PageOnboarding` prompt for unknown slugs. None of these changes affect the architecture documentation content (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
