The diff only changes `app/story-reader.css` — CSS encoding fixes, layout adjustments, and inline comments. None of these changes affect the architecture documentation (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
