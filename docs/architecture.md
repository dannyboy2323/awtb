The diff only changes implementation details within `components/public/StoryReader.tsx` (how the cover image is rendered — switching from the `fill` prop to explicit `width`/`height` with inline styles). None of this affects the architecture documentation content (stack, data flow, cache invalidation, database, email, AI, or key directories).
