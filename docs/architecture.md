The diff only modifies `app/story-reader.css` — CSS layout and styling changes (padding, shadows, box-shadow rules). None of these changes affect the architecture documentation content (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
