The diff only changes CSS layout behavior (how `.desk-hero` aligns items and how `.postcard-container` is positioned on mobile). None of this affects the architecture documentation. The doc is already accurate.
