The diff only changes CSS properties in `app/story-reader.css` (padding, object-fit, text-indent). None of these changes affect the architecture documentation content (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
