The diff only modifies CSS comments and styles in `app/story-reader.css`. None of those changes affect the architecture documentation (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
