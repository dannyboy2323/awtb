The diff only modifies `app/globals.css` — adding section comments, reorganizing duplicate CSS blocks, adjusting mobile postcard sizing, and adding a `margin-top: 25vh` for mobile portrait layout. None of these changes affect the architecture documentation content (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
