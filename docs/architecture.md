The diff only changes CSS layout values in `app/story-reader.css` (gap and height for the story reader). None of this affects the architecture documentation content (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
