The diff changes are limited to the StoryReader component (cover image orientation switching), CSS, Sanity schema, and queries. None of these changes affect the architecture documentation content (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
