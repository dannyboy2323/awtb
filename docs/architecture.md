The diff only changes ESLint's ignored paths (adding `dist/**` and `sanity/**` to `globalIgnores`). This is a linting configuration detail that doesn't affect any content in the architecture documentation (stack, data flow, cache invalidation, database, email, AI, or key directories). The doc is already accurate.
