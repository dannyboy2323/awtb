...
| `tests/unit/epub.test.ts`              | EPUB 3 packaging, image embedding, and failure handling     |
| `tests/unit/epubRoute.test.ts`         | EPUB route validation, CMS mapping, headers, and errors     |
...
| `tests/unit/share.test.ts`             | Required direct and universal sharing destinations          |
...
from regressing. The floating-navigation, sharing, and EPUB files have dedicated
100% statement, branch, function, and line thresholds. The report is generated
by `npm run test:coverage`.
...
| File                        | What it tests                                                      |
| --------------------------- | ------------------------------------------------------------------ |
| `tests/e2e/landing.spec.ts` | Landing nav absence, postcard, story nav, sharing, favorites, EPUB |
